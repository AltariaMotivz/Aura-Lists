...
// --- Build the reCAPTCHA tool *immediately* ---
setupRecaptcha(); 
...
phoneForm.addEventListener('submit', async (e) => {
    ...
    try {
        if (!window.recaptchaVerifier) {
            setupRecaptcha(); // Failsafe if it wasn't ready
        }
...
// --- Friend Modal Listeners (This will work now!) ---
...
// This spell is last, as it relies on all the functions defined above.
onAuthStateChanged(window.auth, (user) => {
    if (user) {
        currentUser = user;
        checkAndCreateUserDoc(user); // Now this spell is safely defined above!
    ...
    // We also reset the reCAPTCHA here, which is good practice.
    setupRecaptcha();
});
